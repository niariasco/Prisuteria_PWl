...
export function UploadImageProductos() {
...
ImageService.createImage(formData)
  .then((response) => {
    console.log("Respuesta imagen:", response);

    const filename = response.data.filename; // <-- CORRECTO

    // Mostrar notificación
    toast.success("Imagen subida: " + filename, {
      duration: 4000,
      position: "top-center",
    });

    // Mostrar imagen si quieres
    const urlImagen = `http://localhost:81/apiprisuteria/uploads/${filename}`;
